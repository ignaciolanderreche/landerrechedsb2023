...
```{r}
  ggplot(mass_shootings,aes(x=location_type,y=total_victims))+geom_boxplot() #very simple and self-explanatory code
```
...
ggplot(set_woVegas,aes(x=location_type,y=total_victims))+geom_boxplot()+theme_minimal()+
  labs(
    title="Distribution of total victims per location type",
    y="Total victims",
    x=NULL
  )
```
...
```
_Answer_:
- Summary statistics don't show any great difference among people with prior mental illness for fatalities, injured nor age. However, the mean victims for people with prior mental ilness is higher than those who don't.
- Viewing the graph, one can notice that clearly the shootings made by people with prior mental ilness is higher, and that the type of places where the shooting occurred too. However, there are many "NA's", which may tell us that the sample size with this attribute  may not be big enough to arrive to conclusions.
...
- From the shootings that had the shooter classified as with or w/o prior mental ilness, most of the victims and shootings are registered to the ones that had a prior mental ilness.
...
card_fraud %>% 
  ...
  mutate(freq_in_percentage=total_trans/sum(total_trans)*100)
...
card_fraud %>% 
  ...
  mutate(freq_in_percentage=total_amount/sum(total_amount)*100)
```
...
```
_Answer_ It doesn't seem like fraud is related to distance!!
...
-   Who did you collaborate with: Only wiht myself
-   Approximately how much time did you spend on this problem set: Around 5 hours
-   What, if anything, gave you the most trouble: Labels and calculating frequencies
